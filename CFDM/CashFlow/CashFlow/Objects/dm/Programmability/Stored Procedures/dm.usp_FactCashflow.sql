...
from stg.FactCashflow
except
select distinct [workmatternumber]
from stg.Adjuster)),

Policynumber as (
select distinct f.claim_Exposure_No,
fa.policy_No,
fa.portfolio_Cd,
fa.Business_Type_Cd,
fa.NAIC_Cd,
...
from [stg].[FactCashflow] f left join [stg].FactActuals fa 
on f.claim_Exposure_No=fa.claim_Exposure_No
)
...
from stg.FactCashflow F left join dm.DimClaim C on SUBSTRING (F.claim_Exposure_No,0,CHARINDEX('-',F.claim_Exposure_No))=c.claimNo
...
left join Dm.DimPolicy p  on pa.policy_No=p.policyNumber and pa.portfolio_Cd=p.portfolioCode and
pa.[NAIC_Cd]=p.NAICCode and pa.[Business_Type_Cd]=p.businessTypeCode
