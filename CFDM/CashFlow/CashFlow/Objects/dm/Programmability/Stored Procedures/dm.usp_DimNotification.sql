CREATE PROCEDURE [dm].[usp_DimNotification]
...
DECLARE @Ins_proc_id INT= 3

--- Insert the NA value SK
SET IDENTITY_INSERT [dm].[DimNotifications] ON 

INSERT INTO [dm].[DimNotifications]
([notification_SK],[workMatterNo],[Comments],[Confidence],[Status],[audit_Insert_Dt],[audit_Update_Dt],[ins_Proc_ID],
[upd_Proc_ID],[is_Deleted],[is_Active])


VALUES (0, 'NA','NA','NA','NA',@InsertDate,NULL, @Ins_proc_id,NULL,0,1)

SET IDENTITY_INSERT [dm].[DimNotifications] OFF 

-- Load the data from stg to datamart
INSERT INTO [dm].[DimNotifications]([workMatterNo],[Comments],[Confidence],[Status],[audit_Insert_Dt],[audit_Update_Dt],[ins_Proc_ID],
[upd_Proc_ID],[is_Deleted],[is_Active])
