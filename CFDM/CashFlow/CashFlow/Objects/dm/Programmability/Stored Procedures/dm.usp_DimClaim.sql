CREATE PROCEDURE [dm].[usp_DimClaim]
...
DECLARE @Ins_proc_id INT= 4

--- Insert the NA value SK
SET IDENTITY_INSERT [dm].[DimClaim] ON 

INSERT INTO [dm].[DimClaim] ([claim_SK],[claimNo],[actuarialSegment],[fairfaxSegment],[claimSegment],
...
SET IDENTITY_INSERT [dm].[DimClaim] OFF 
...
-- Load the data from stg to datamart
 ;with claim AS(
	...
	SELECT 
    distinct
    (SUBSTRING (claim_Exposure_No,0,CHARINDEX('-',claim_Exposure_No))
    ) claimno
FROM 
	[stg].[FactCashflow]
...
INSERT INTO [dm].[DimClaim] ([claimNo],[actuarialSegment],[fairfaxSegment],[claimSegment],[audit_Insert_Dt]
...
from claim C left Join [stg].[Claim] CS on C.claimNumber=CS.claimNumber
